  ...
  const rows: TableRow[] = [
    ...
    ...Object.keys(documentsPresents).map((key, i) => {
      ...
      return new TableRow({ children: [
        ...
        dCell(analysis?.findings || '—', COL[2], { shade: i % 2 === 1 }),
...
function buildGenderAssessment(data: any): (Paragraph | Table)[] {
  ...
  const objRows: TableRow[] = [
    ...
    ...(ga.objectives ?? []).map((o: any, i: number) => new TableRow({ children: [
  ...
  const qtRows: TableRow[] = [
    ...
    ...Object.entries(ga.quantitative_data ?? {}).map(([k, v]: [string, any], i) => new TableRow({ children: [
  ...
  const consRows: TableRow[] = [
    ...
    ...(ga.consultations ?? []).map((c: any, i: number) => new TableRow({ children: [
  ...
  const envRows: TableRow[] = [
    ...
    ...envImpacts.map((imp: any, i: number) => {
      ...
      return new TableRow({ children: [
        ...
        dCell(imp.severity ?? '—', ENV_COL[4], { shade: i % 2 === 1, center: true, bold: true, color: sColorK ? sevColor[sColorK] : C.text }),
  ...
  const socioRows: TableRow[] = [
    ...
    ...socioImpacts.map((imp: any, i: number) => new TableRow({ children: [
      ...
      dCell(imp.opportunity ?? '—', SOCIO_COL[4], { shade: i % 2 === 1 }),
  ...
  const recRows: TableRow[] = [
    ...
    ...(ga.recommendations ?? []).map((r: any, i: number) => new TableRow({ children: [
  ...
  return [
    ...
    ...(envImpacts.length > 0 ? [
      ...
      new Table({ width: { size: TW, type: WidthType.DXA }, columnWidths: ENV_COL, rows: envRows }),
      spacer(200),
    ] : []),
    // ── Impacts socioéconomiques (NOUVEAU) ──
    ...(socioImpacts.length > 0 ? [
      ...subTitle('Impacts socioéconomiques différenciés'),
      new Table({ width: { size: TW, type: WidthType.DXA }, columnWidths: SOCIO_COL, rows: socioRows }),
      spacer(200),
    ] : []),
...
function buildComplaintMechanism(data: any): (Paragraph | Table)[] {
  ...
  const docRows: TableRow[] = [
    ...
    ...Object.entries(cm.documentary_basis ?? {}).map(([k, v]: [string, any], i) => new TableRow({ children: [
      dCell(k,              DOC_COL[0], { shade: i % 2 === 1, bold: true }),
      dCell(v.finding,      DOC_COL[1], { shade: i % 2 === 1 }),
      dCell(v.evidence ?? '—', DOC_COL[2], { shade: i % 2 === 1 }),  // ← evidence
  ...
  const critRows: TableRow[] = [
    ...
    ...Object.entries(cm.key_criteria ?? {}).map(([k, v]: [string, any], i) => new TableRow({ children: [
  ...
  const recRows: TableRow[] = [
    ...
    ...(cm.recommendations ?? []).map((r: any, i: number) => new TableRow({ children: [
      dCell(r.recommendation, REC_COL[0], { shade: i % 2 === 1 }),
      dCell(r.priority,       REC_COL[1], { shade: i % 2 === 1, center: true, bold: true }),
  ...
  return [
    ...
    ...(cm.strengths ?? []).length === 0
      ? [new Paragraph({ spacing: { after: 60 }, children: [new TextRun({ text: '—', size: 20, font: 'Calibri', color: C.muted, italics: true })] })]
      : (cm.strengths ?? []).map((s: string) => new Paragraph({
...
export async function generateFormDataWordDocument(data: any): Promise<Buffer> {
  ...
  const projectName = data.project_name ?? 'Projet';
...
function buildCritereTable(items: any[]): (Paragraph | Table)[] {
  ...
  const rows: TableRow[] = [
    ...
    ...items.map((item, i) => new TableRow({ children: [
      ...
      dCell(item.sources_methode ?? '—',    COL[2], { shade: i % 2 === 1 }),
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Table({
            ...
            ] as [string, string][]).map(([label, val], i) => new TableRow({ children: [
    ...
    sections: [
        ...
        children: [
          ...
          new Table({
            ...
            rows: ([
              ...
              ['Personne rencontrée', data.personne_rencontree  ?? '—'],
      ...
      },
      {
        ...
        children: [
          ...
          kvParagraph("Durée de l'entretien", data.duree_entretien  ?? '—'),
          ...
          new Paragraph({
            ...
            indent: { left: 280 },
            children: [new TextRun({ text: data.commentaires_libres || '—', size: 20, color: C.text, font: 'Calibri', italics: true })],
          }),
          spacer(240),
          ...sectionTitle('13', 'Signature'),
          kvParagraph("Signature de l'auditeur", data.signature_auditeur ?? '—'),
          spacer(240),
        ],
      },
    ],
...
function buildThemeTable(theme: any, showNuisances = false): (Paragraph | Table)[] {
  ...
  const rows: TableRow[] = [
    ...
    ...questions.map((q: any, i: number) => {
      ...
      if (showNuisances) {
        return new TableRow({ children: [
          dCell(q.questionId ?? q.question_id, COL[0], { shade, center: true }),
...
export async function exportGuideEntretienWord(data: any): Promise<Buffer> {
  const generatedAt  = new Date().toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' });
  const guideType    = data.guide_type ?? '';
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Table({
            width: { size: 7000, type: WidthType.DXA },
            columnWidths: [2800, 4200],
            rows: infoRows.map(([label, val], i) => new TableRow({ children: [
              new TableCell({ width: { size: 2800, type: WidthType.DXA }, shading: { fill: C.primary }, margins: CELL_MARGINS, children: [new Paragraph({ children: [new TextRun({ text: label, bold: true, color: C.white, size: 20, font: 'Calibri' })] })] }),
              new TableCell({ width: { size: 4200, type: WidthType.DXA }, shading: { fill: i % 2 === 0 ? C.lightGray : C.white }, margins: CELL_MARGINS, children: [new Paragraph({ children: [new TextRun({ text: val, size: 20, color: C.text, font: 'Calibri' })] })] }),
            ]})),
          }),
      ...
      },
      {
        ...
        children: [
          ...
          ...(data.theme4 ? [
            ...sectionTitle('04', theme4Titles[guideType] ?? 'Thème 4'),
          ...
          new Paragraph({
            ...
            border: { left: { style: BorderStyle.SINGLE, size: 12, color: C.accent, space: 20 } },
            indent: { left: 280 },
            children: [new TextRun({ text: data.notes_auditeur || '—', size: 20, color: C.text, font: 'Calibri', italics: true })],
          }),
          spacer(240),
        ],
      },
