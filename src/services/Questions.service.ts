...
export class QuestionService extends BaseRepository {
  ...
  async getAvailableSections(projectId: string, formType: FormType): Promise<string[]> {
    const [rows] = await pool.query<any[]>(
      `
    ...
    return rows.map(r => r.section_key as string);
  }
